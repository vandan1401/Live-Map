...
- **The per-row "Backdrop" button from the entry below is gone; backdrop is now reached
  ...
  session as each other, superseding parts of the plan-30 entry directly below (its
  migration/RLS/function work all stands unchanged — this only moves *where* the UI lives
  and adds a second, JSON-driven way to set the values):
  1. **docs/plans/31.md:** owner — "the backdrop should lie in upload colony where we
     upload json and svg" — `ColonyPicker.tsx`'s per-row "Backdrop" button and `onBackdrop`
     prop are removed; `ColonyBackdropScreen.tsx` moved from `features/colony-picker/` to
     `features/colony-upload/` (its only remaining caller) to keep the cross-feature-import
     rule intact. `ColonyUploadScreen.tsx`'s `upload()` now chains a successful
     create/replace straight into that screen (a `{kind: "backdrop", colonyId, intro}`
     stage) instead of closing — reachable, prefilled, no separate "done" message.
  2. **docs/plans/32.md:** owner, after seeing the running app — "everything that lives
     after enable public link... i want that to live in colony.json file only" — then,
     separately, "also make changes in pipeline... to enter the required values in json."
     `contract/colony.schema.json` gained an optional `colony.backdrop` object (sibling to
     `select_zoom`, `attribution` required); when a manifest declares it,
     `ColonyUploadScreen.tsx` calls a new `lib/colony/colonyBackdrop.ts::applyManifestBackdrop()`
     on every create/replace, **authoritative and unconditional** — it overwrites whatever
     the colony currently has. Omitting `backdrop` from the manifest is the only way to
     leave an existing alignment untouched (`applyManifestBackdrop`'s `applied: false` path,
     never calls `updateColonyBackdropTransform` at all). Closing the loop the owner's
     second ask pointed at: `tools/pipeline`'s `ColonyConfig` (the hand-maintained
     `colonies/<id>.json` input, never regenerated by a run) gained the same optional
     `backdrop` field, and `build_manifest()` copies it into every export verbatim — a plain
     passthrough with nothing in the drawing to compute, constrain, or validate it against
     (`select_zoom` looks similar but is genuinely derived from `COL-ZOOM-REF`, not passed
     through). **This is a one-way door once used**: once a colony's
     config declares `backdrop`, that file is the value's home, not the database — an
     alignment tweak made afterwards in the app's backdrop form must be copied back into the
     config or the next export/upload reverts it (documented in `contract/SPEC.md`,
     `docs/cad-layer-standard.md`, the schema's own `description`, and a line in
     `ColonyBackdropScreen.tsx`'s own hint text — a `/review` finding on this plan, since the
     first three drafts of that documentation asserted "omit the key" as the *only* opt-out
     without saying the config file itself is now the source an operator must edit).
     A first attempt at the JSON-declaration half was built and reverted earlier the same
     session — see this file's prior `## Deferred` entry (now resolved) for exactly what
     `/review` caught and how this build closed each finding.
  **Verified:** `mingw32-make -C tools/pipeline verify` 133 passed/1 skipped (was 129,
  the 4 new backdrop tests); `mingw32-make contract` 4/4; apps/map typecheck/lint clean;
  `ColonyUploadScreen.test.tsx`/`colonyBackdrop.test.ts`/`colonyBackdropRls.test.ts`
  19/19 (two new tests assert the exact `.update()` payload and its absence, not a status
  string); `pnpm build` clean, `dist/` clean of `admin-portal`; full `mingw32-make gate`
  test run 281/286 — the 5 failures are the same documented pre-existing baseline (4
  anon-grant-drift + 1 `subscribePlots` flake), none touching this diff. **Correction
  against this entry's own first draft:** `mingw32-make -C tools/pipeline golden`'s "1
  skipped" is `tests/test_golden.py`'s longstanding stub (`/review` flagged the same stub
  2026-08-24; still open, unrelated to this diff) — it does not actually compare exported
  bytes against `fixtures/shree-vatika-2/`, so it cannot be cited as proof the fixture is
  byte-identical. The real evidence the shared fixture is untouched: `git status` shows no
  change under `fixtures/shree-vatika-2/`, and the new backdrop tests use their own temp
  colony config, never the shared one.
  **Not verified, not achievable from here (no browser):** a real upload through the
  browser with a `backdrop`-bearing `colony.json`, and a real pipeline export against a
  `colonies/<id>.json` declaring one. Plan 32 stays open (no `Status: complete` marker)
  until the owner does both.
  **Next:** owner verifies the two items above in the running app/a real `make export`;
  separately, applies the three pending backdrop migrations to production (see
  `## Deferred`'s entry below — unrelated to and not blocked by this change).
...
- **Colony backdrop also reachable and editable from inside the app itself, not only the
  ...
  **Not verified, not achievable from here (no browser):** the new "Backdrop" button and
  ...
  marker) until the owner does that. **Superseded in part by the entry directly above:**
  the per-row "Backdrop" button this entry introduced was removed by docs/plans/31.md; the
  screen and both lib functions this entry built are otherwise unchanged and still the
  underlying write path.
  **Next:** owner applies this migration to production after the two from plan 29
  (PROGRESS.md's own Deferred entry has the full checklist and ordering).
...
- ~~Owner ask, 2026-09-12, not built: let `colony.json` declare a colony's backdrop
  ...
  authoritative for `backdrop` whenever the manifest declares it, on every create and every
  replace, no exceptions** — an operator who wants a colony's UI-tuned alignment left alone
  on a routine geometry-fix replace simply omits `backdrop` from that upload's manifest
  (`applyManifestBackdrop()`'s `applied: false` early return, `lib/colony/colonyBackdrop.ts`,
  tested by `ColonyUploadScreen.test.tsx`'s "never calls update()..." case). And the
  invariant 6 gap — `build_manifest()` never emitted `backdrop`, so a hand-added block was
  silently discarded on the next `make export` — is closed for real, not accepted: the
  owner's follow-up ask the same session ("also make changes in pipeline... to enter the
  required values in json") added an optional `backdrop` field to
  `tools/pipeline/pipeline/extract/types.py`'s `ColonyConfig`, read by `load_colony_config()`
  from the hand-maintained `tools/pipeline/colonies/<id>.json` (never regenerated by a run,
  unlike `colony.json` itself) and copied through verbatim by `build_manifest()` — the same
  place-in-the-config-file shape `select_zoom` uses, though unlike `select_zoom` (derived
  from `COL-ZOOM-REF`) this value is a plain passthrough with nothing to compute. The
  operator now maintains one file, once; every future export carries it forward
  automatically. `attribution` is required in the schema (no
  `?? ""` anywhere it's read) so an omitted credit fails validation instead of silently
  blanking the ODbL column — the exact bug the reverted attempt shipped.
...
### 2026-09-13 — Colony backdrop declared in colony.json, pipeline propagates it (docs/plans/31.md + 32.md, Tier 1, D-049/D-123)
- Done: closed out plan 31 (backdrop screen moved into the upload flow, chained onto a
  successful create/replace, per-row picker button removed) and plan 32 (`colony.backdrop`
  added to `contract/colony.schema.json`, applied unconditionally on every upload via
  `applyManifestBackdrop`, and `tools/pipeline`'s `ColonyConfig`/`build_manifest` now pass
  the same field through verbatim). Carried over uncommitted from the previous session —
  this session re-ran verification fresh and ran one more `/review` pass before committing.
- Next: none open on this plan. Plans 31 and 32 both stay without a `Status: complete`
  marker on purpose — both have a browser-only acceptance criterion (a real chained
  upload/backdrop flow, and a real pipeline export against a `backdrop`-declaring
  `colonies/<id>.json`) that isn't achievable from here.
- Surprises: the final `/review` pass (after everything else was already verified clean)
  caught a real ODbL attribution error the worked example in `docs/cad-layer-standard.md`
  and `contract/SPEC.md` would have shipped: it used the OSM *tile* credit string instead
  of this project's actual Produced-Work notice (`docs/plans/28.md`'s own D-049-adjacent
  attribution line, chosen after a 2026-09-08 review finding on the same string). Since
  D-049/D-123 make that example config the operator's copy-paste source once a colony
  declares `backdrop` in JSON, a wrong string there would have propagated for real. Fixed
  before commit.
- Verified: `mingw32-make -C tools/pipeline verify` 133 passed/1 skipped; `golden` 1
  passed/1 skipped (pre-existing stub, not byte-identity proof); `mingw32-make contract`
  4/4, re-run after the attribution fix; `apps/map` `pnpm typecheck`/`lint`/`build` clean;
  `pnpm test -- --run` 282/286, the 4 failures the documented pre-existing anon-grant-drift
  baseline, none touching this diff; `dist/` confirmed free of `admin-portal`; `git status`
  on `fixtures/shree-vatika-2/` empty (shared fixture untouched).
