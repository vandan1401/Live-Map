...
| Selected-plot overlay: raise, per-edge dimension callout, auto pan/zoom (M6, owner-requested; rewritten 2026-08-21/22 for real, non-axis-aligned plots) | `apps/map/src/lib/colony/plotGeometry.ts` — pure, DOM-free (`parsePlotPoints`, rotating-calipers `minAreaRect`, `simplifyNearCollinear`, `polygonCentroid`) | none | `components/useSelectedPlotOverlay.ts` (hook, called once from `ColonyMap.tsx`), `components/plotDimensionOverlay.ts` (draws one dashed line+label per real polygon edge, not an abstracted length/breadth pair — see plotGeometry.ts for the geometry) | none |
| Plot-number label orientation/size (2026-08-21) | none | none | `components/alignPlotLabels.ts`, called once at mount from `useColonyMapMount.ts` — reads `data-rotation`/`data-label-height` off each `.plot-label` (baked in by `pipeline/export/svg.py` from the source DXF's own label rotation/height, docs/plans/17.md) and applies them as `transform`/inline `font-size`; both optional, falls back to upright/CSS-default size when absent (e.g. `fixtures/shree-vatika-2/colony.svg`, which predates this and has neither) | none |
...
| `parseColonySvg(raw)` | `apps/map/src/components/parseColonySvg.ts` | Parses raw SVG markup into a mounted-ready `SVGSVGElement` — adds the dimension-arrow marker def and the garden/road pattern defs. Split out of `ColonyMap.tsx` (docs/plans/11.md /review finding — a component file may only export components) so `ColonyUploadScreen.tsx`'s preview renders the same decorated SVG the map itself does, not a bare parse with no fill for roads/gardens. |
