...
| `parsePublicToken(hash)` | `apps/map/src/lib/colony/publicLinkUrl.ts` | Pure — matches `#/public/<uuid>` (strict uuid grouping, not just "36 hex/dash chars", per `/review` 2026-08-31) and returns the token or `null`. `App.tsx` checks this before the `session` gate — a public-link visitor never needs to sign in. |
