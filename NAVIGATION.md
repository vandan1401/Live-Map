...
| `assign_plot_numbers(rings, labels, config)` → `PlotMatchResult` | `tools/pipeline/pipeline/matching/assign.py` | Plot identity: regex-validates each label (`^([A-Z]+-)?[0-9]+$`, rejects dimension strings), resolves block (default = `config.blocks[0]`, explicit prefix must be in `config.blocks`), checks `config.number_range` and `config.number_width` independently (a number can be in-range and still too wide to pad — that's an error, not a truncation), zero-pads, builds `svg_id`, and errors on duplicate `svg_id`s across rings. `default_block_count`/`explicit_block_count` on the result is spec/12's required "report the split". |
