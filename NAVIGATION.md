...
| `pnpm generate-public-link <colony_id>` | `apps/map/scripts/generate-public-link.ts` | docs/plans/22.md phase 2's entire "admin surface" for public links — generates a new random token (`crypto.randomUUID()`), sets `colonies.public_token`, prints the `#/public/<token>` hash to append to the app's own origin. Not idempotent by design — rerunning it regenerates (and invalidates) the token. Revocation is hand-run SQL (`update colonies set public_token = null where id = '<id>'`), not a script, matching plan 21's precedent for a genuinely rare admin action. |
