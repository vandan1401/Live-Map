...
| Command | Does |
|---|---|
| `CV-LAYERS` | creates the 9 `COL-*` layers (including optional `COL-ZOOM-REF`, docs/plans/20.md) + 3 `CV-*` scratch layers |
...
| Per-colony click-to-focus zoom reference (`COL-ZOOM-REF`, docs/plans/20.md) | `tools/pipeline/pipeline/extract/dxf.py` (`_read_zoom_ref`, ingest), `pipeline/export/normalise.py` (`ring_extent_px`, measurement) / `apps/map/src/components/map/view.ts` (`computeSelectZoom`, `useFlyToSelectedPlot.ts`, render) | 1 |
...
|---|---|---|---|---|
| Colony render, pan/zoom (M1; rewritten to canvas 2026-08-22, docs/plans/18.md, D-027; per-colony click-to-focus zoom 2026-08-29, docs/plans/20.md) | `apps/map/src/components/map/colonyModel.ts` (SVG text → plain draw model; **no `Path2D`, no `getBBox`** so it parses under jsdom), `plotPicker.ts` (point-in-polygon, replaces DOM hit-testing), `view.ts` (pure view maths + `colonyLatLngBounds`/`leafletViewState`, the **only** place SVG space is bound to Leaflet; also `selectZoomFor`/`computeSelectZoom`) | none | `components/map/useColonyCanvas.ts` (Leaflet init, canvas layer, attachSync, picking, repaint scheduling) → `colonyCanvasLayer.ts` (an `L.Layer` owning a **viewport-sized** canvas — never colony-sized, that is the whole performance fix) → `drawColony.ts` + `drawLabels.ts` + `drawDimensions.ts`; textures in `canvasPatterns.ts`; colours resolved from CSS in `colonyTheme.ts`; `ColonyMap.tsx` owns React state and chrome only; `useFlyToSelectedPlot.ts` (split out for invariant 7, same reason as `usePlotDimensions.ts`) owns the fly-to-plot effect and computes the zoom via `computeSelectZoom` — falls back to the fixed `SELECT_ZOOM` for a colony with no `COL-ZOOM-REF` reference | none |
...
| Auth: username/password + RLS lockdown (docs/plans/09.md, M8); org-scoped RLS added on top (docs/plans/21.md phase 1, 2026-08-31) | `apps/map/src/lib/auth/{username,session}.ts` (D-019, D-020) | none new — sessions/reads go through the existing `lib/db/browserClient.ts` client; `scripts/create-user.ts` is the only writer of `auth.users`, via the service-role key, and now requires an org id (4th CLI arg, no default) | `App.tsx` owns the single app-lifetime Supabase client and the session gate (`getSession`/`onAuthStateChange`), replacing the old `!actor` gate; `features/auth/LoginScreen.tsx` (username/password form, shown whenever there is no session) | `auth.users` (via Admin API only, `app_metadata.org_id` set at creation); `organizations` (new table, RLS select-only scoped to the caller's own org); RLS on `colonies`/`plots`/`plot_history` is select-only, authenticated-**and-own-org**-only (`org_id uuid not null`, denormalized on all three) — every security-definer RPC (`apply_plot_transition`, `bulk_set_initial_plot_data`, `create_colony_from_manifest`) independently re-checks org ownership internally, since RLS does not apply inside a `security definer` function body; see `apply_plot_transition()`'s own comment for why writes still work at all |
...
| `ingest_dxf(dxf_path, config)` | `tools/pipeline/pipeline/extract/dxf.py` | M10 (D-118): reads a conforming DXF's modelspace into `DxfIngestResult` (rings + labels + resolved `north_deg` + optional `zoom_ref`), strict — every rejection names the layer and entity handle, never repairs or guesses. `ezdxf` types never cross past this module (tier-2.md, "format code stays at the edge"). `COL-ZOOM-REF` (docs/plans/20.md, 0 or 1) accepts a plain closed `LWPOLYLINE` or a block `INSERT` (resolved via `virtual_entities()`) — rejects a rotated insert outright rather than silently mismeasuring it. |
...
| `match_labels_to_rings(noun, rings, labels, *, allow_unmatched_labels=False)`, `MatchingError` | `tools/pipeline/pipeline/matching/__init__.py` | M12 (spec/12, `docs/plans/13.md`): the one containment test (via `pipeline.geom.contains`) shared by `assign.py` and `classify.py` — pairs each ring with the single label whose insertion point it contains, raising `MatchingError` naming the entity handle(s) for any zero/multi match either direction. The old contained→nearest→flag ladder is deleted; there is no distance-threshold fallback anywhere in this module. `allow_unmatched_labels` (docs/plans/19.md, 2026-08-24) lets `classify.py` alone skip a label matching zero rings instead of raising — a `COL-FEATURE-NO` text over the road with no containing feature ring is a road/pathway annotation, not an error. `assign_plot_numbers` never passes it; plot-number matching stays exactly as strict as before. |
...
| `ring_extent_px(t, ring)` | `tools/pipeline/pipeline/export/normalise.py` | docs/plans/20.md: width/height of a ring's bounding box in SVG-viewBox px — translation-invariant (unlike `apply_transform`, which maps one point). Assumes the ring is axis-aligned; used only for the optional `COL-ZOOM-REF` reference rectangle's `select_zoom.{ref_width_px,ref_height_px}` in the manifest. |
...
| `build_svg(..., feature_labels)` | `tools/pipeline/pipeline/export/svg.py` | M13: assembles `colony.svg` per `contract/SPEC.md` — zero styling attributes outside its own fallback `<style>` block, which is scoped with `svg:root ` so it applies only when the file is opened standalone, never once the app inlines it into the live DOM (a real `/review` finding, 2026-08-21: an unscoped block out-cascaded `colony-theme.css`). Trees are `<use class="tree" href="#tree-canopy">` with explicit `width`/`height` (never omit — defaults to 100% viewport, already happened once) and a `class="tree-crown"` `<circle>` symbol def. Each plot's visible `plot-label` text is bare `int(plot.number)`, prefixed with `{block}-` only when the colony mixes more than one distinct `block` (docs/plans/16.md) — a single-block colony's labels are unchanged; this is independent of `svg_id`, which never collides regardless. `feature_labels` (docs/plans/19.md, 2026-08-24) emits one `<text class="feature-label">` per `COL-FEATURE-NO` label — matched to a ring or not — at its own DXF point with `data-rotation`/`data-label-height` mirroring `plot-label`'s (real DWG font size/rotation, not a fixed constant); text is XML-escaped (free-form CAD-operator text, unlike a plot number). `_HIDDEN_FEATURE_KINDS` (currently `{park, reserved, other}`, owner-tuned 2026-08-24) withholds a matched feature's `<text>` only — its `<path>`/`data-kind`/manifest entry are untouched, so un-hiding a kind is a one-line edit there, never a re-export; a road/pathway annotation (no ring, no kind) is never affected by that set. |
...
| `buildGrassPattern(ctx, image)`, `buildRoadPattern(ctx, baseColor)`, `buildRoadEdgePattern(ctx, baseColor)` | `apps/map/src/components/map/canvasPatterns.ts` | Canvas patterns for the map's ground/road textures, replacing the SVG `<pattern>` defs on 2026-08-22. Same owner-supplied photo, same 2x2 mirror scheme, same 160x144 tile and 1-unit seam overlap — a mirrored tile has no seam regardless of whether the source photo tiles. Baked once into an offscreen canvas, then `createPattern`. The separate world-ground layer is gone: the painter simply fills the visible world rect with ground first, so texture never runs out however far you pan. `buildRoadEdgePattern` (docs/plans/19.md, 2026-08-24) is a small light-grey fleck tile stroked (never filled) along the road path's own outline — the "like pathway" inner trim, since the contract has no separate pathway class to target. |
| `drawLabels(ctx, model, theme, state, bounds)` | `apps/map/src/components/map/drawLabels.ts` | Draws plot/feature/entrance labels (measured with `measureText`, no `getBBox` and so no layout dependency — the old SVG version had to defer a frame for that). No background chip since 2026-08-23 (owner reference) — feature labels render in `--colony-feature-label-ink` directly, which is why that token is light now. **Culls to the viewport, which is required, not an optimisation:** un-culled labels measured 20.6ms of a 37.5ms frame. Every size here is in SVG user units, matching the CSS it replaced. Size falls back to `PLOT_LABEL_DEFAULT_SIZE`/`FEATURE_LABEL_SIZE` only when the label carries no `data-label-height` (docs/plans/19.md addendum, 2026-08-24) — owner wants the DWG's own font size read verbatim when the pipeline supplied one, for a feature-label same as a plot-label. |
| `fillDecor(ctx, model, theme, state)`, `pathFor(shape)` | `apps/map/src/components/map/drawDecor.ts` | Split out of `drawColony.ts` for invariant 7's 250-line cap (/review, 2026-08-24). Static decoration (site boundary, road — now with `buildRoadEdgePattern`'s inner-edge stroke — garden, amenity, water) plus the shared per-shape `Path2D` cache (`pathFor`), reused by `drawColony.ts` for plots too. |
...
| `parseNullablePaise(value)` | `apps/map/src/shared/parsePaise.ts` | The one place a free-text paise string is validated (D-010) — a regex gate, not just `Number.isInteger(parseInt(...))`, so `"12.5"` is rejected rather than silently truncated. Used by `scripts/import-seed.ts`; no longer by the CSV bulk-import (see below). Unit-tested in `parsePaise.test.ts`. |
| `parseSimpleBulkImportCsv(raw, plots)`, `parseSimpleBulkImportRows(rows, plots)` | `apps/map/src/lib/colony/parseBulkImportFile.ts` | Pure CSV parser for the initial-data import (docs/plans/10.md, format simplified 2026-08-24 per owner ask). Only two columns are read — plot, then owner name — everything else in the sheet is ignored; status is derived (owner present → `booked`, blank/`NMC` → `available`), never read from the file. Takes the colony's plot identities (`fetchPlotsByColony`) to resolve a sheet's plot label to a real `svg_id`, case-insensitively; an unmatched or duplicate plot is skipped and reported, never rejects the whole file. `parseSimpleBulkImportRows` takes pre-split cells so an XLSX adapter (not yet built, see PROGRESS.md Deferred) can share it. Unit-tested in `parseBulkImportFile.test.ts`. |
...
| `createColonyFromManifest(client, manifest, svg, replace)` | `apps/map/src/lib/colony/createColonyFromManifest.ts` | Domain wrapper around `callCreateColonyFromManifest` (`lib/db/colonies.ts`), the only place `create_colony_from_manifest` is called via `.rpc()`. `features/colony-upload/ColonyUploadScreen.tsx` is the only caller. Live-integration tests in `createColonyFromManifest.test.ts`. Since docs/plans/20.md also passes through `manifest.colony.select_zoom?.{ref_width_px,ref_height_px}` as two more RPC params (`undefined` when absent). |
| `selectZoomFor(viewport, refWidthPx, refHeightPx)`, `computeSelectZoom(viewport, refWidthPx, refHeightPx, minZoom, maxZoom)` | `apps/map/src/components/map/view.ts` | docs/plans/20.md: the per-colony equivalent of the old fixed `SELECT_ZOOM` constant. `selectZoomFor` is the raw fit-zoom math (same `min(width-ratio, height-ratio)` shape as `fitScale`, then `log2`); `computeSelectZoom` wraps it with the SELECT_ZOOM fallback (when a colony has no `COL-ZOOM-REF`) and the `[minZoom, maxZoom]` clamp — pure and unit-tested without a live Leaflet map, unlike the effect that calls it. |
| `fetchPublicColony(client, token)` | `apps/map/src/lib/db/colonies.ts` | docs/plans/22.md phase 2: the only place `.rpc("get_public_colony", ...)` is called. No session required — the token is the entire authorization boundary (no `org_id` check, by design). |
| `loadPublicColony(client, token)` | `apps/map/src/lib/colony/publicColony.ts` | Thin DOM-free wrapper around `fetchPublicColony`. `PublicColonyResult` (`lib/db/types.ts`) is `{found: false} \| {found: true, colony: {id, name, svg}, plots: {svg_id, status}[]}` — an explicit, hand-written column list on the RPC's SQL side (never `select *`), so no PII/money column can leak by accident when one is added to `plots`/`colonies` later. |
| `parsePublicToken(hash)`, `buildPublicLinkHash(token)` | `apps/map/src/lib/colony/publicLinkUrl.ts` | Pure — matches `#/public/<uuid>` (strict uuid grouping, not just "36 hex/dash chars", per `/review` 2026-08-31) and returns the token or `null`. `App.tsx` checks this before the `session` gate — a public-link visitor never needs to sign in. `buildPublicLinkHash` is the inverse, used by `ShareLinkButton.tsx` to build the copyable link from a colony's already-fetched `public_token` (owner ask, 2026-09-01). |
| `ShareLinkButton` | `apps/map/src/features/colony-picker/ShareLinkButton.tsx` | Reads a colony's existing `public_token` (already on `ColonyRow`, no new query) and copies the full shareable URL to the clipboard — "No public link yet" if none. Generating/revoking the link is deliberately **not** done from here — that stays an admin-only action via `admin-portal/` or `scripts/generate-public-link.ts`; this button only ever reads and shares. Rendered per-row in `ColonyPicker.tsx`. |
| `renderColonyPreview(container, svg, statuses?)` | `apps/map/src/components/map/renderColonyPreview.ts` | Still, non-interactive render of a colony for the upload screen's confirmation. Shares the model, theme, patterns and painter with the live map **deliberately**: that confirmation is the only code path writing `verified: true` (D-025, invariant 2), so it must show what the map will show. `statuses` (docs/plans/22.md phase 2) is optional and defaults to today's all-`"available"` behaviour when omitted — `ColonyUploadScreen.tsx`'s call is unchanged; `PublicColonyView.tsx` is the only caller that supplies real statuses. |
| `PublicColonyView` | `apps/map/src/features/public-colony/PublicColonyView.tsx` | docs/plans/22.md phase 2: the unauthenticated, per-colony, token-scoped read-only page — plot status only, via `renderColonyPreview`. No search/table/detail-sheet/realtime (see the plan's Non-goals). A transport error and a `found: false` result render distinct messages (`/review` 2026-08-31 — conflating them told an offline visitor their link was revoked). |
| `regeneratePublicLink(client, colonyId)`, `revokePublicLink(client, colonyId)` | `apps/map/src/lib/colony/publicColony.ts` | docs/plans/23.md phase 3: promoted out of `scripts/generate-public-link.ts`'s own inline logic — the script and the admin portal's `server.ts` are now both callers of one implementation. `regeneratePublicLink` is not idempotent by design (regenerating invalidates the previous token); `revokePublicLink` is. |
| `fetchOrganizations(client)`, `insertOrganization(client, name)` | `apps/map/src/lib/db/organizations.ts` | docs/plans/23.md phase 3: the admin portal's org read/create surface. `.from()` lives here per tier-2.md's layer rule. |
| `fetchColoniesByOrg(client, orgId)` | `apps/map/src/lib/db/colonies.ts` | docs/plans/23.md phase 3: the admin portal's colony list for one org — includes unverified colonies too (unlike `fetchVerifiedColonies`), since the portal's operator is an admin, not a map viewer. |
| `listOrgUsers`, `createOrgUser`, `reassignUserOrg` | `apps/map/admin-portal/actions.ts` | docs/plans/23.md phase 3, D-032: the admin portal's own domain layer (outside `src/`, never part of the shipped Vite bundle — confirmed by grepping `dist/`). `createOrgUser`/`listOrgUsers`/`reassignUserOrg` call `client.auth.admin.*` directly (same precedent `create-user.ts` and `testHelpers.ts`'s `createScratchUser` already established for that API surface — the "`.from()` only in `lib/db`" rule doesn't extend to it). `reassignUserOrg` fetches-then-merges `app_metadata` as defense in depth, though GoTrue's Admin API was measured to already merge it server-side (`/review` 2026-08-31). |
| Admin portal server | `apps/map/admin-portal/server.ts` + `static/{index.html,portal.js,portal.css}` | docs/plans/23.md phase 3, D-032: local-only (`127.0.0.1:5002`), plain `node:http`, no login layer, `Content-Type: application/json` required on every mutating route (CSRF mitigation). Run via `pnpm admin-portal`/`make admin-portal` — **owner-run only, guard-hook-blocked from Claude** (`.claude/hooks/guard.sh`, mirroring `make ui`/`make serve`). |
...
| `pnpm generate-public-link <colony_id>` | `apps/map/scripts/generate-public-link.ts` | Thin CLI wrapper around `regeneratePublicLink` (`lib/colony/publicColony.ts`) — printing/exit-code shape is this script's own; the token-generation logic itself is shared with the admin portal (docs/plans/23.md). Not idempotent by design — rerunning it regenerates (and invalidates) the token. |
| `pnpm admin-portal` / `make admin-portal` | `apps/map/admin-portal/server.ts` | docs/plans/23.md phase 3's admin portal — see its own row above. **Owner-run only** — `.claude/hooks/guard.sh` blocks Claude from starting it. |
