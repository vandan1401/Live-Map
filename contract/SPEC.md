...
**`backdrop` is optional and, when present, authoritative** (D-036/D-047/D-048/D-049) — the
alignment for this colony's synthetic-aerial backdrop, applied to the database on every
upload/replace that includes it, overwriting whatever the colony currently has. Omit the
key entirely on a routine geometry-fix replace to leave an existing backdrop untouched —
that is the only way to say "leave it". `attribution` is required, not optional, so an
omitted credit is a validation error rather than a silently blanked column. **`tools/pipeline`
cannot derive this block** — a manually aligned aerial photo has no geometric source — but
it does propagate one: declare it once in `tools/pipeline/colonies/<id>.json` (the
hand-maintained per-colony config `make export` reads, never regenerated by a run) and
every future export carries it through into `colony.json` verbatim. Unlike `select_zoom`
above, which is genuinely derived from `COL-ZOOM-REF`, `backdrop` is a plain passthrough of
whatever the config declares — nothing in the drawing computes, constrains, or validates
it. The actual backdrop image is a separate upload — JSON cannot carry binary data —
attached through the app's own backdrop screen after the colony is live.
**Once `colonies/<id>.json` declares `backdrop`, that config file is the value's home, not
the database** — an adjustment made afterwards in the app's own backdrop form must be
copied back into that config, or the next `make export`/upload will silently revert it to
the stale declaration. Deleting the block from the config is how to hand the value back to
the UI permanently.
